...
def random_nodes(m: int, ordered_indexes: bool = False) -> list[mapping.node]:
    """
...
class TestTSP(unittest.TestCase):
    ...
    def test_tsp(self) -> None:
        ...
        nodes: list[mapping.node] = [
            (0, 0, 0), (1, 2, 3), (2, 2, 7), (3, 3, 3)]

        for index, neighbors in enumerate(matrix):
            matrix[index] = list(
                sorted(neighbors, key=lambda x: mapping.node_distance(x, nodes[index])))
    ...
    def test_find_next_destination(self) -> None:
        for _ in range(100):
            nodes: list[mapping.node] = random_nodes(1000, True)
            next_valid_index: int = random.randint(600, 995)
            visited: set[mapping.index] = set(
            ...
            with self.assertRaises(ValueError):
                nodes: list[mapping.node] = random_nodes(1000, True)
                visited: set[mapping.index] = set(range(0, 1000))
                index, x, y = tsp.find_next_destination(nodes, visited)

    def test_distance(self) -> None:
        self.assertAlmostEqual(mapping.node_distance((0, 0, 0), (1, 3, 4)), 5)
