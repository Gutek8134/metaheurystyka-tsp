...
class TestInstanceGenerator(unittest.TestCase):
    def test_generator(self):
        ...
        for size in random.sample(range(100, 10000), 50):
            instance = generator.random_instance(
                size, min_x=min_x, max_x=max_x, min_y=min_y, max_y=max_y)
        ...
        min_x = 1000
        self.assertRaises(ValueError, lambda: generator.random_instance(
            10000, min_x=min_x, max_x=max_x, min_y=min_y, max_y=max_y))

        min_x = 1
        self.assertRaises(ValueError, lambda: generator.random_instance(
            100000, min_x=min_x, max_x=max_x, min_y=min_y, max_y=max_y))
